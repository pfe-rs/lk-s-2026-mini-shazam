...
class CNNFingerprinter(AbstractFingerprinter):
    ...
    def fingerprint(self, spectrogram: np.ndarray) -> np.ndarray:
        ...
        spec_tensors = torch.from_numpy(spectrogram).float()
        spec_tensors = spec_tensors.unsqueeze(0).unsqueeze(0)

        print(spec_tensors.shape)

        spec_tensors = spec_tensors.to(self.device)
        with torch.no_grad():
            embedding = self.model(spec_tensors)
